...
(when (maybe-require-package 'ivy)
  ...
  (defun zenk/enable-ivy-flx-matching ()
  ...
  (when (maybe-require-package 'projectile)
    (let ((search-function
      ...
      (when search-function
        (defun zenk/counsel-search-project (initial-input &optional use-current-dir)
...
instead."
    ...
    (global-set-key (kbd "M-?") 'zenk/counsel-search-project)))


(when (maybe-require-package 'swiper)
  (after-load 'ivy
    (defun zenk/swiper-at-point (sym)
      "Use `swiper' to search for the symbol at point."
      (interactive (list (thing-at-point 'symbol)))
      (swiper sym))

    (define-key ivy-mode-map (kbd "M-s /") 'zenk/swiper-at-point)))
